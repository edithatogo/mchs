...
def test_emergency_grouper_placeholders_are_not_implementation_evidence() -> None:
    ...
    assert reference["implementation_evidence"] is False
    assert "placeholder-only" in str(reference["evidence_status"])
    assert precomputed["output_status"] == "synthetic"
    assert precomputed["implementation_evidence"] is False
    assert "synthetic-placeholder-only" in str(precomputed["evidence_status"])
    assert _as_mapping(precomputed["provenance"])["classification_basis"] == (
