...
def test_power_platform_binding_declares_service_api_boundary_not_formula_logic():
    ...
    assert "connector-boundary validation" in metadata["description"]
    ...
    assert "Power Platform-owned calculation logic" in fail_example[
        "rejected_pattern"
    ]
...
def test_power_platform_env_vars_connection_refs_and_alm_gates_are_local_safe():
    ...
    alm_gates = {
        gate["name"]: gate for gate in _as_sequence(contract["alm_gates"])
    }
    ...
    assert "pytest tests/test_power_platform_binding_track.py" in alm_gates[
        "contract-validation"
    ]["tool"]
    assert "scripts/validate_power_platform_capabilities.py" in alm_gates[
        "contract-validation"
    ]["tool"]
...
def test_power_platform_docs_point_to_concrete_contract_artifacts():
    ...
    assert "mchs_api_default_calculator_id" in env_vars
    assert "mchs_api_default_pricing_year" in env_vars
    assert "mchs_fixture_gate" in env_vars
