...
TRACK = ROOT / "conductor" / "archive" / "stata_interop_binding_20260513"
...
def test_stata_interop_track_metadata_docs_and_contract_bundle_are_conservative():
    ...
    assert (
        metadata["pre_archive_current_state"]
        == "file-cli-adapter-ready-with-publication-gates"
    )
    ...
    for phrase in [
        ...
        (
            "Document package publication only after fixture and "
...
def test_stata_interop_scaffold_is_thin_and_non_formula():
    scaffold_root = ROOT / "bindings" / "stata"
