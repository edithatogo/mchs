...
def test_license_workflow_track_docs_and_contract_are_complete() -> None:
    ...
    assert metadata["current_state"] in {
        "implemented-local-only-workflow",
        "complete-with-gaps",
    }
...
def test_contract_examples_are_synthetic_and_local_only() -> None:
    ...
    assert asset.get("implementation_evidence", False) is False
    assert "placeholder" in str(asset.get("evidence_status", "placeholder-only"))
