...
```matlab
% CLI/file boundary example; calculator execution remains in the shared core.
...
- C ABI MEX integration requires a Rust `cdylib` build and a MATLAB compiler
  (`mex`) configuration. This path is a future native support gate, not a
  current MATLAB interop support claim.
