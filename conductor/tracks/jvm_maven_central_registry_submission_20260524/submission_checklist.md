...
2. Wait for Central to discover public signing key `BB03C82343A653EE44BD5CDA9DF6B142F065199E` from a supported keyserver, or publish the key through another supported path.
...
3. Create a fresh short-lived Central user token for the upload session.
4. Re-upload `build/mchs-jvm-bindings-0.1.0-central-bundle.zip` through Central Portal or the Publisher API.
5. Release the deployment after Central validation succeeds.
6. Record the release/deployment identifier, submission URL, and any reviewer notes.
7. Verify the public metadata exposes version `0.1.0`.
   ```text
