...
- ConanCenter review cleanup: fork commit `0e7d9052` changes the C smoke test pointer comparison from `0` to `NULL`; commit `d3a07060` switches the test_package runner to `self.cpp.build.bindir` for multi-config and Windows portability; static and shared `conan create` validations passed after the change.
