...
- [x] Task: Submit to `General registry` using an authenticated publisher workflow. Replacement registry PR submitted after reviewer naming feedback.
    ...
    - [x] Capture submission URL, version, owner, and review state. PR is open with checks running and AutoMerge stopwatch pending. The prior `NwauCore` path was abandoned after reviewer naming feedback.
- [x] Task: Conductor - Automated Review and Checkpoint 'Submission' (Protocol in workflow.md). Pending General checks, review, AutoMerge waiting period, merge, and registry propagation.

## Phase 4: Publication Evidence

- [ ] Task: Verify external publication. Pending General merge and registry propagation.
    - [x] Query public registry after propagation. Current JuliaHub query for `NationalWeightedActivityUnitWrapper` returns 404 Not Found.
    - [x] Record immutable URL/API response/checksum. General PR `https://github.com/JuliaRegistries/General/pull/156254`; repository `https://github.com/edithatogo/NationalWeightedActivityUnitWrapper.jl`; tag `v0.1.0`; commit `56ddec5ae29513e80717d4625f82c024a211c949`; Registrator confirmation `https://github.com/edithatogo/NationalWeightedActivityUnitWrapper.jl/issues/1#issuecomment-4535378120`; release notes `https://github.com/edithatogo/NationalWeightedActivityUnitWrapper.jl/issues/1#issuecomment-4535380457`.
