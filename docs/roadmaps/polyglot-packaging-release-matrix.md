...
| Julia | Package wrapper over the shared core or file contract | `NationalWeightedActivityUnitWrapper 0.1.0` has active General PR #156254 from repo `https://github.com/edithatogo/NationalWeightedActivityUnitWrapper.jl`, tag `v0.1.0`, commit `56ddec5ae29513e80717d4625f82c024a211c949`, PR head `bb63b2a81ec2ded2c5675f09fb6cd63128f10a07`, UUID `58dad789-f56a-4ab3-a66f-c15139bf9cbe`; checks successful; pending 3-day new-package AutoMerge wait; superseded `NwauCore 0.1.0` General PR #156236 is preserved as superseded evidence | `private` | Release when Julia CI, artifact install checks, and parity fixtures pass without runtime-specific behavior drift, the active General PR merges, and registry publication evidence exists. |
...
| JVM / Maven Central | Kotlin/JVM binding jar over file/service boundary contracts | `io.github.edithatogo:mchs-jvm-bindings 0.1.0` builds locally with required POM metadata, jar, sources jar, javadoc jar, signatures, checksum sidecars, and a prepared Central Portal bundle; Sonatype Central namespace `io.github.edithatogo` is verified; Maven Central metadata probe returns 404/no publication | `private` | Release when publisher credentials and in-memory PGP signing are configured, the signing public key is discoverable, an authenticated Central Portal upload/release succeeds, and public Maven metadata verifies `0.1.0`. |
| Swift | Swift Package Manager package over C ABI, service, or file contract | PackageList issue closed as completed, but public SPI listing/version evidence remains pending | `private` | Release when Apple-platform fixtures pass, native packaging does not create duplicated calculator behavior, and SPI public evidence exists. |
| Stata | Stata ado/do examples or package over file, CLI, or service contract | Prepared ado/help/pkg bundle; no SSC publication evidence | `private` | Release when health-economics examples pass shared fixtures and remain boundary-only. |
| MATLAB | MATLAB scripts/toolbox over file, CLI, service, or C ABI contract | Prepared upload bundle; no File Exchange publication evidence | `private` | Release when numerical examples pass shared fixtures and no MATLAB formula implementation is introduced. |
