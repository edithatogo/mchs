...
| Open VSX / Visual Studio Marketplace | `mchs-tools` | Open VSX API `https://open-vsx.org/api/edithatogo/mchs-tools` returns `edithatogo.mchs-tools` version `0.1.1` after workflow `https://github.com/edithatogo/mchs/actions/runs/27457810800`; Visual Studio Marketplace remains public at version `0.1.0` under publisher `edithatogo`; Open VSX token was configured for publication, but `VSCE_PAT` is not configured for the Marketplace `0.1.1` republish | complete for Open VSX `0.1.1` and Marketplace `0.1.0`; configure `VSCE_PAT` or manually upload the `0.1.1` VSIX before claiming Marketplace `0.1.1` |
