...
| Open VSX / Visual Studio Marketplace | `mchs-tools` | extension source scaffold committed; manifest is publisher-ready; VSIX rebuild on 2026-06-13 produced SHA-256 `9dfddc420f795f7037c9b8721649548f67e40ecb992473c124952bf86a6a8aeb`; public Open VSX API probe for `edithatogo/mchs-tools` still returned 404; `ovsx publish mchs-tools-0.1.0.vsix` still prompted for a namespace PAT instead of publishing; `vsce publish --packagePath mchs-tools-0.1.0.vsix` failed PAT verification with `TF400813`; generated `.vsix` intentionally not committed; 2026-06-13 audit found no local OVSX/VSCE/AZURE_DEVOPS credential env vars and only `NUGET_API_KEY` configured in GitHub secrets | needs valid `OVSX_PAT` for namespace `edithatogo`; Visual Studio Marketplace publisher/PAT access remains unresolved |
