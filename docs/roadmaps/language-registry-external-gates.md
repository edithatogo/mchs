...
| Open VSX / Visual Studio Marketplace | `mchs-tools` | Open VSX API `https://open-vsx.org/api/edithatogo/mchs-tools` now returns `edithatogo.mchs-tools` version `0.1.1` after workflow `https://github.com/edithatogo/mchs/actions/runs/27457810800`; Visual Studio Marketplace remains public at version `0.1.0` under publisher `edithatogo` | partial publication verified; configure `VSCE_PAT` or manually upload the `0.1.1` VSIX before claiming both extension registries are synchronized |
