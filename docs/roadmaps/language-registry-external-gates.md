...
| Julia General | `NationalWeightedActivityUnitWrapper` | reviewer-requested replacement PR #156254 is open; registry consistency, treecheck, AutoMerge, and AutoMerge-stopwatch checks pass; superseded PRs marked with `[noblock]` comments because close permission was denied | General PR #156254 merge and public registry propagation |
...
| conda-forge | `nwau-py` | PR `https://github.com/conda-forge/staged-recipes/pull/33452` is open and mergeable; latest head `04452d8f9b839493acedbabf198140b562648ac9`; linter, check-skip, aggregate staged-recipes, linux_64, osx_64, and win_64 checks all pass | maintainer review, merge, feedstock publication, and public Anaconda propagation |
...
| SSC / Stata package distribution | `mchs` (`mchs-stata-interop` bundle) | ado/help bundle submitted by email; Gmail message id `19e5ffd483ef5841` | SSC maintainer review and public archive publication |
| vcpkg / ConanCenter | `nwau-c-abi` | dedicated source archive exists; Conan static/shared builds and native `test_package` pass after NULL cleanup; ConanCenter fork commit `657c7a31` makes Cargo profile and shared/static library selection explicit for current Copilot review threads; vcpkg overlay install passes locally with CMake config target and linkage-aware artifacts; upstream vcpkg and ConanCenter PRs are open | ConanCenter maintainer job approval/review plus CLA; vcpkg Rust/Cargo toolchain policy plus Microsoft CLA before publication can be claimed |
