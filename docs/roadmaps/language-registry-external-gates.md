...
| Open VSX / Visual Studio Marketplace | `mchs-tools` | Visual Studio Marketplace publisher page shows `MCHS Tools` version `0.1.0` as Public under publisher `edithatogo`; public Marketplace extension query for `edithatogo.mchs-tools` returns publisher `edithatogo`, extension `mchs-tools`, display name `MCHS Tools`, and version `0.1.0`; merged publish workflow dry-run `https://github.com/edithatogo/mchs/actions/runs/27455041574` packaged the VSIX successfully; public Open VSX API probe for `edithatogo/mchs-tools` still returned 404; generated `.vsix` intentionally not committed; 2026-06-13 audit found no local OVSX/VSCE/AZURE_DEVOPS credential env vars and only `NUGET_API_KEY` configured in GitHub secrets | Visual Studio Marketplace complete for `0.1.0`; Open VSX still needs valid `OVSX_PAT` for namespace `edithatogo` |
