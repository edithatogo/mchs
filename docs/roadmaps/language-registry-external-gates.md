...
| Open VSX / Visual Studio Marketplace | `mchs-tools` | Open VSX public API verifies `edithatogo.mchs-tools` version `0.1.1`; Visual Studio Marketplace remains public at `0.1.0`; Open VSX token was configured for publication, but `VSCE_PAT` is not configured for the Marketplace `0.1.1` republish | complete for Open VSX `0.1.1` and Marketplace `0.1.0`; configure `VSCE_PAT` before claiming Marketplace `0.1.1` |
