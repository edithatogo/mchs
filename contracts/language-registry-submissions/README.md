...
- Publications verified: PyPI `nwau-py`, crates.io `nwau-core`, NuGet `Mchs.Bindings.DotNet`, npm `@edithatogo/mchs-wasm-binding`, Homebrew personal tap, Go module proxy/pkg.go.dev, and the official MCP registry submission tracked outside this language-registry contract.
...
Remaining blockers are Open VSX Eclipse-GitHub account linking plus publisher tokens, Visual Studio Marketplace PAT setup, Maven Central publisher credentials/signing/upload/release, upstream maintainer review workflows, public semantic tags/indexing, vcpkg policy deferral plus ConanCenter upstream review, and unavailable local proprietary runtimes such as MATLAB/Stata.
