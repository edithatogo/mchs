...
- Prepared artifact: `/tmp/mchs-tools-0.1.1.vsix` packaged from `integrations/vscode` with `npx --yes --package @vscode/vsce vsce package --no-dependencies --out /tmp/mchs-tools-0.1.1.vsix`; workflow packaging still writes `microcosting_healthservices/integrations/vscode/mchs-tools.vsix` when using the explicit `--out` command.
