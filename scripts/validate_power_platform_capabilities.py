...
def source_evidence_exists(source_path: str, manifest_paths: set[str]) -> bool:
...
def validate() -> None:
    ...
    raw_manifest = json.loads(RAW_ARCHIVE_MANIFEST.read_text(encoding="utf-8"))
    require(
        isinstance(raw_manifest, list), "raw IHACPA archive manifest must be a list"
    )
    manifest_paths = {
        entry["path"]
    ...
    for calculator_id, row in calculators.items():
        ...
        for pricing_year, source_path in source_evidence.items():
            ...
            require(
                source_evidence_exists(source_path, manifest_paths),
                (
                    f"{calculator_id} {pricing_year} source evidence missing: "
                    f"{source_path}"
                ),
            )
            require(
    ...
    for calculator_id in {"acute", "ed", "subacute", "outpatients"}:
        ...
        for pricing_year in {str(year) for year in range(2013, 2026)}:
            require(
                ...
                (
                    f"{calculator_id} {pricing_year} must be visible across "
                    "the archive horizon"
                ),
            )
            require(
    ...
    ed_variants = {
        ...
        for variant in calculators["ed"]["variant_surfaces"]
    }
    require(
    ...
    for calculator_id in {"adjustment", "hac"}:
        ...
        for pricing_year in {str(year) for year in range(2018, 2026)}:
            require(
                row["year_states"][pricing_year]
                in {"source_available", "implemented", "helper"},
                (
                    f"{calculator_id} {pricing_year} must be visible across "
                    "the archive horizon"
                ),
    ...
    require(
        ...
        (
            "capabilities example does not preserve source-available disabled "
            "state wording"
        ),
    )
    require(
